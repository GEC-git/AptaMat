...
def affinity_propagation(distance_matrix, standard, sigma=np.arange(1, 10, 0.1)):
    ...
    for v in standard:#.values():
        standard_labels.append(cn * v)# += [cn] * v
    ...
    for sigma in sigma_iter:
        ...
        clustering = AffinityPropagation(damping=0.52, affinity="precomputed", convergence_iter=30, max_iter=2000,
...
affinity_matrix, aff_prop_clust_best, aff_prop_calinski_best, silhouette_best, acc_best, sigma_best, sub_aff_prop = \
    affinity_propagation(dist_matrix, [0 for i in range(291)])#np.arange(5, 15, 0.1))
...
### Heatmap setup
df = pd.DataFrame(dict_label)#index=list(set(labels)), columns=dict_label.keys())
s = df.sum()
...
ax.set_xticks(np.arange(0, len(family_np)), labels=list(np.arange(0, len(family_np))))
for i in range(len(df.columns)):
    for j in np.arange(0, len(family_p_t[i])):
...
plt.text(len(family_np)+1, 1, 'Occupancy (%)', fontsize=14)
